...
@section('title', __('menus.backend.accounts.purchaseorder.edit') . ' #'.$purchaseorder->id . ' :: ' . app_name())
@role('seller')
...
@endif
@section('content')
    {{ html()->form('PUT')->id('form_purchase_order_edit_submit')->class('form-horizontal')->open() }}
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-5">
                        <h4 class="card-title mb-0">
                            @lang('menus.backend.accounts.purchaseorder.edit')
                ...
                <div class="form-group row">
                                    <label class="col-md-2 form-control-label" for="stock_id">Stocks <span style="color:red">*</span></label>
                                    <div class="col-md-10">
                                        <select class="form-control select2" name="stock_id" id="stock_id" maxlength="191">
                                            <option value="">Select Stock</option>
                                            @foreach($stockid as $stock)
                                            <option {{old('stock_id')==$stock->id?'selected':''}} value="{{ $stock->id}}" @if(@$stock->id == @$purchaseorder->stock_id) selected @endif>{{ $stock->id}}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-10">
                                        <select class="form-control select2" name="buyer_id" id="buyer_id" maxlength="191">
                                            ...
                                            <option {{old('buyer_id')==$buyer->id?'selected':''}} value="{{ $buyer->id}}" @if(@$buyer->id == @$purchaseorder->buyer_id) selected @endif>{{ $buyer->username}}</option>
                                            @endforeach
                                        </select>
                                        <div class="invalid-feedback"></div>
                                    </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-10">
                                        <select class="form-control select2" name="seller_id" id="seller_id" maxlength="191">
                                            ...
                                            <option {{old('seller_id')==$seller->id?'selected':''}} value="{{ $seller->id}}" @if(@$seller->id == @$purchaseorder->seller_id) selected @endif>{{ $seller->name}} ({{ $seller->username}})</option>
                                            @endforeach
                                        </select>
                                        <div class="invalid-feedback"></div>
                                    </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-3">
                                    {{ html()->text('delivery_date')->class('datepicker form-control')->value(@$purchaseorder->delivery_date)->placeholder('Delivery Date')->attribute('maxlength', 191) }}
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                @endif
                ...
                <div class="form-group row">
						<label class="col-md-2 form-control-label" for=""></label>
					...
					<div class="form-group">
						...
						<select name="truck[{{$truck->id}}][load_status]" id="truck[{{$truck->id}}][load_status]" class="select2 form-control">
...
<script type="text/javascript">
    ...
    $('#form_purchase_order_edit_submit').on('submit', function(event) {
      event.preventDefault();
      var formData = new FormData($(this)[0]);
      $.ajax({
        url: "{{ route($route_pre.'.purchaseorder.update', $purchaseorder->id) }}",
        method: 'POST',
        data: formData,
        contentType: false,
        cache: false,
