...
@section('title', 'Edit Postal Code :: ' . app_name())
@section('content')
    {{ html()->form('PUT')->id('form_postal_code_edit_submit')->class('form-horizontal')->open() }}
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-sm-5">
                    ...
                    <div class="col">
                        ...
                        {{ html()->label('Postal Code Short')->class('col-md-2 form-control-label')->for('postal_code_short') }}
                            <div class="col-md-3">
                                {{ html()->text('postal_code_short')
                                    ...
                                    ->attribute('maxlength', 191)
                                    ->value($postalcode->postal_code_short)
                                }}
                            </div><!--col-->
                        </div><!--form-group-->
                      ...
                      {{ html()->label('Phone Code')->class('col-md-2 form-control-label')->for('ph_code') }}
                        ...
                        <div class="form-group row">
                          ...
                          <div class="col-md-3">
                            <input type="number" name="price" value="{{$postalcode->price}}" data-decimals="2" step="0.1"/>
                          </div><!--col-->
                        </div><!--form-group-->
            ...
            <div class="card-footer clearfix">
                <div class="row">
                    <div class="col">
                        {{ form_cancel(route('admin.postalcodes.index'), __('buttons.general.cancel')) }}
                    </div><!--col-->
...
@push('after-scripts')
 <script type="text/javascript">
    ...
    $('#form_postal_code_edit_submit').on('submit', function(event) {
      event.preventDefault();
      var formData = new FormData($(this)[0]);
      $.ajax({
        url: "{{ route('admin.postalcodes.update', $postalcode->id) }}",
        method: 'POST',
        data: formData,
        contentType: false,
        cache: false,
