            ...
            <div class="row">
                <div class="col-md-12" id="tabs">
                    ...
                    @if(isset($productPrefRel))
                        ...
                        @foreach(@$productPrefRel as $SPkey=>$productSpecRel)
                            <div id="pref{{$tabNumber}}" class="tab-pane {{ ($tabNumber ==1)?'active':'' }} product-group">
                                <table class="table table-bordered">
                                    <tr>
                                     ...
                                     <tr>
                                        @foreach(array_keys($productProdRel[$SPkey]) as $key=>$value)
                                            ...
                                            @if($value=="product_id")
                                            <td><label data-id="old_product[{{$SPkey}}][product_name]">
                                                  {{@$products[$productProdRel[$SPkey][$value]]}}</label> <input type="hidden" name="old_product[{{$SPkey}}][product_name]" value="{{$productProdRel[$SPkey][$value]}}"> </td>
                                            @elseif($value=="delivery_country")
                                               <td><label data-id="{{$value}}[{{$SPkey}}]">{{@$countryNameList[$productProdRel[$SPkey][$value]]}}</label> <input type="hidden" name="{{$value}}[{{$SPkey}}]" value="{{$productProdRel[$SPkey][$value]}}"></td>
                                            @else
                                            <td><label data-id="{{$value}}[{{$SPkey}}]">{{@$productProdRel[$SPkey][$value]}}</label> <input type="hidden" name="{{$value}}[{{$SPkey}}]" value="{{$productProdRel[$SPkey][$value]}}"></td>
                                            @endif
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <button type="button" class="btn btn-edit editItem" data-id="{{$SPkey}}">
                                    ...
                                    @php
                                        $i=0;$pref_id=$SPkey;$index1=0; $list = array_keys($productProdRel[$SPkey]);
                                    @endphp
                                    @foreach($productSpecRel as $pKey=>$productSpec)
                                    @php print_r($productSpec['default']); @endphp

                                @if($productSpec['hasmany'] == 'No')
                                  <input type="hidden" name="specification[{{$pref_id}}][{{$pKey}}]" value="{{@$productSpec['default']}}">
                                @else
                                @if(isset($productSpec['options']))
                                 @if(count($productSpec['options']) > 0)
                                  ...
                                  @if($productSpec['buyer_pref_anylogic'] == 'Yes')
                                    <input class="switch-input any_type_selected" type="hidden" >
                                   @endif
                                  @if(count($productSpec['options']) > 12)

                                    <input name="accept_all[{{$pref_id}}][{{$pKey}}]" class="switch-input any_type_selected" type="hidden"  @if(in_array('all', $productSpec['default']??array())) value="1" @endif >


                                    @if(!empty($productSpec['default']))
                                      @if(is_array($productSpec['default']))
                                        @if(count($productSpec['default'])>0)
                                          @php foreach($productSpec['default'] as $productSpecificationName) { @endphp
                                              <input type="hidden" name="specification[{{$pref_id}}][{{$pKey}}][]" value="{{@$productSpecificationName}}">
                                          @php } @endphp
                                        @endif
                                      @endif
                                    @endif

                                    <input type="hidden" name="premium_single[{{ $pref_id }}][{{$pKey}}]" value="{{@$productSpec['options'][@$premium_key]['premium']?? 0}}"  data-decimals="0" min="-15" max="15" step="1"/>

                                 @else
                                  @foreach($productSpec['options'] as $key => $option)
                                    @php $option_key = str_slug($option['name'], "_"); @endphp
                                          <input type="hidden" name="specification[{{$pref_id}}][{{$pKey}}][{{$option_key_num}}]" />
                                          <input type="hidden" name="premium[{{ $pref_id }}][{{$key}}]" value="{{@$option['premium']?? 0}}"  data-decimals="0" min="-15" max="15" step="1"/>
                        ...
                        @php $tab_key = $SPkey; $tab_key++; $tab_key_heading2++;$tabNumber++; @endphp
                        @endforeach
                    @else
                            ...
                            <div class="col-md-12" id="tabs">
                                <form id="prefadd" method="get" role="form">
                                    <div class="tab-content">
                                        <div id="pref1" class="tab-pane active product-group">
                                            <div class="form-group row">
                                              <input type="hidden" name="product_pref_id" id="product_pref_id" />
                                                {{ html()->label('<strong>Product</strong>')->class('col-md-2 form-control-label')->for("old_product[1][product_name]") }}
                                                <div class="col-md-10">
                                                {{ html()->select("old_product[1][product_name]")->id("product_1_product_name")
                                                ->class('select2 form-control products-details')
                                                ->options($products)
                                                ->value(@$buyer->product->id)
                                                ->attribute('data-pref-id', $tab_key)
...
<script type="text/javascript">
    ...
    let productlist = JSON.parse('@json($productProdRel)');
    ...
    $(document).on('click','.editItem',function(){
        ...
        $("#product_pref_id").val(id);
        $("#product_1_product_name").attr("data-pref-id",id);
        $("#product_1_product_name").attr("data-edit-id",1);
        $("#createPrefModal").modal("show");
        $("#product_1_product_name").attr("name","old_product["+id+"][product_name]");
        $("#product_1_product_name").val(productlist[id].product_id);
        $("#product_1_product_name").trigger("change");
    });

    function setAddressInformation(){
        if(id!="") {
            $(document).find("[name='delivery_city["+id+"]']").val(productlist[id].delivery_city);
            $(document).find("[name='delivery_postalcode["+id+"]']").val(productlist[id].delivery_postalcode);
            $(document).find("[name='delivery_street["+id+"]']").val(productlist[id].delivery_street);
            $(document).find("[name='delivery_country["+id+"]']").val(productlist[id].delivery_country).trigger("change");
            id="";
        }
    }
    $(document).on('click','#AddProductSpecification',function(){
        let product_pref_id = $("#product_pref_id").val();
        let form = $("#prefadd").serializeArray();
        if(product_pref_id!="") {
            for (let i = 0; i < form.length; i++) {

                if(!$(".table").find("[name='"+form[i].name+"']").length){
                    $("#products_specs").prop("tagName");
                    $("#products_specs").after(`<input type='hidden' name='${form[i].name}' value='${form[i].value}' / >`);
                }
                let name = form[i].name.substr(0,form[i].name.indexOf("[")).replace(/_/g, " ");
                if(name=="delivery country"){
                    $(".table").find("[data-id='"+form[i].name+"']").html(countryNameList[form[i].value]);
                }else{
                    $(".table").find("[data-id='"+form[i].name+"']").html(form[i].value);
                }
                $(".table").find("[name='"+form[i].name+"']").val(form[i].value);
            }
        }
        else
        {
            ...
            tableContent +=`<div id="preft${tabNumber}" class="tab-pane product-group active"><table  id="table${tabNumber}" class='table display responsive nowrap table-bordered data-table'>`;
            let columnList = [];
            tableContent +=`<tr>`;
            for (let i = 0; i < form.length; i++) {
                if(form[i].value!=""){
    ...
    $(".pro-btnAdd").click(function(){
        ...
        $("#createPrefModal").modal("show");
        tabNewNumber++;
        $("#product_1_product_name").attr("data-pref-id",tabNewNumber);
        $("#product_1_product_name").attr("name","product["+tabNewNumber+"][product_name]");
        tabn++;
        tab_key_heading++;
    });
    ...
    $('button[type="submit"]').on('click',function(){
        ...
        let activetab_focusid='';
        let inactivetab_focusid='';
