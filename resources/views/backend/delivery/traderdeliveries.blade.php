...
@section('title', 'Trader Deliveries Card View :: '.app_name())
@push('after-styles')
<style>       
    .carousel-indicators
    {
    ...
    <div class="row" id = "viewcards">
        <div class="col-md-2 col-sm-12 mb-4">
            <h4 class="card-title mb-0">
                ...
                <a href="{{ route($route_pre.'.deliveries.index') }}" class="btn btn-info btn-md ml-1" title="Trader Deliveries List View" data-toggle="tooltip" ><i class="fas fa-list"></i></a>
            </h4>
        </div><!--col-->         
            ...
            @foreach($deliveries as $delivery)
                <div class="col-xs-12 col-sm-6 col-md-4 removeable">
                    <div class="card">
                        ...
                        <a class="btn btn-edit" href="{{ route('trader.deliveries.edit', $delivery->id) }}"> <i class="fas fa-edit"></i></a>
                        </div>
                    </div>
                </div>
...
// $('#load_more').click(function(){
    ...
    $('.filters').change( function() {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {'last_id':0, 'productid':fp, 'variety':fv, 'quatity':fq, 'sortby': sortby},
                success: function(data)
                {
                    var result = JSON.parse(data);
                        if(result.status == 'success')
     ...
     $('#variety').change( function() {
            ...
            $.ajax({
                ...
                data: {'last_id':0, 'productid':fp, 'variety':fv, 'quatity':fq},
                success: function(data)
                {
                    var result = JSON.parse(data);
                        if(result.status == 'success')
