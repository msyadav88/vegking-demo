...
@section('title', __('labels.backend.access.roles.management') . ' | ' . __('labels.backend.access.roles.edit'). ' | ' . app_name())

@section('content')
{{ html()->modelForm($role, 'PATCH')->id('form_roleedit_submit')->class('form-horizontal')->open() }}
    <div class="card">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-5">
                            ...
                            <div class="row">
                                    @foreach($groupItems as $groupItemKey=>$groupItemName)
                                        <div class="col-md-3 checkbox d-flex align-items-center">
                                             {{ html()->label(
                                                html()->checkbox('permissions[]', in_array($groupItemName, $rolePermissions), $groupItemName)
                                                        ->class('switch-input')
                        ...
                        <div class="row">
                           @if(($permissionArr))
                                @foreach($permissionArr as $permissionName=>$permissionId)
                                    <div class="col-md-3 checkbox d-flex align-items-center">
                                        {{ html()->label(
                                                html()->checkbox('permissions[]', in_array($permissionName, $rolePermissions), $permissionName)
                                                        ->class('switch-input')
...
@push('after-scripts')
  <script type="text/javascript">
    ...
    $('#form_roleedit_submit').on('submit', function(event) {
      event.preventDefault();
      var formData = new FormData($(this)[0]);
      $.ajax({
        url: "{{  route('admin.auth.role.update', $role) }}",
        method: 'POST',
        data: formData,
        contentType: false,
        cache: false,
