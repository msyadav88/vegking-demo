...
@section('title', 'Edit Transport Country Season :: ' . app_name())
@section('content')
                            ...
                            <div class="col-md-10">
                                <select class="form-control" id="fromTo" name="fromTo">
                                    ...
                                    @foreach($fromtoopt as $key=>$value)
                                        @php
                                            if($key == $season->fromTo){
                                                $selectedfrt = 'selected';
                                            }else{
                                                $selectedfrt = '';
                                            }
                                        @endphp
                                        <option value="{{$key}}" {{$selectedfrt}}>{{$value}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
@push('after-scripts')
 <script type="text/javascript">
    ...
    $('#seasonform').on('submit', function(event) {
      ...
      $.ajax({
        url: "{{ route('admin.season.update', $season->id) }}",
        method: 'POST',
        data: formData,
        contentType: false,
        cache: false,
