...
@if($productSpec['field_type'] == 'dropdown_switchboxes')
  @if($productSpec['hasmany'] == 'No')
    <div class="form-group row nets-append">
      ...
      <div class="col-md-10">
        <select class="select2 form-control" name="stock[{{ $pref_id }}][specification][{{ $pKey }}]" id="stock[{{ $pref_id }}][specification][{{ $pKey }}]" maxlength="191" @if(@$productSpec['can_edit'] == "No") disabled="disabled" @endif >
	...
	<div class="form-group row entity nets-append">
		...
		@php $option_key = str_slug($option['name'], "_"); @endphp
			<div class="flex_item form-group">
				<div class="checkbox switch-box d-flex align-items-center">
						{{ html()->label(
						html()->checkbox('stock['.$pref_id.'][specification]['.$pKey.']['.$option_key_num.']', in_array($key, $productSpec['default']??array()), $key)
						->class('switch-input switch_select_item')
						->attribute('data-group','entity')
						->id('stock['.$pref_id.'][specification]['.$pKey.']['.$option_key_num.']')
						. '<span class="switch-slider" data-checked="on" data-unchecked="off"></span>')
						->class('switch switch-label switch-pill switch-primary mr-2')
						->for('stock['.$pref_id.'][specification]['.$pKey.']['.$option_key_num.']')
						}}
               @if(!empty(@$option['ec']))
...
@elseif($productSpec['field_type'] == 'inputfield')
    <div class="form-group row">
        ...
        <div class="col-md-10">
            {{ html()->text('stock['.$pref_id.'][specification]['.$pKey.']')
      ...
      <div class="col-md-10">
        <div class="form-group row">
          <div class="col-md-2 col-sm-6" style="position:relative">
            ...
            <input class="form-control" type="text" value="{{@$stock->size_from ?? '45'}}" placeholder="Min" name="stock[{{ $pref_id }}][specification][{{ $pKey }}][size_from]"  /> <span style="position:absolute;bottom: 7px;right: 20px;">mm</span>
          </div>
          <div class="col-md-2 col-sm-6" style="position:relative">
            <label class="form-control-label" for="size_to">To</label>
            <input class="form-control" type="text" value="{{@$stock->size_to ?? '65'}}" placeholder="Max" name="stock[{{ $pref_id }}][specification][{{ $pKey }}][size_to]"  /> <span style="position:absolute;bottom: 7px;right: 20px;">mm</span>
          </div>
        </div>
      </div><!--col-->
    </div><!--form-group-->
