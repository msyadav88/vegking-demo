 ...
 <div class="loading loading_hide"></div>
  ...
  <!--  <a data-fancybox data-src="#agreePopup" href="javascript:;">
    ...
    <div id="app" class="@yield('classes', '')">
        ...
        @include('frontend.includes.v2.nav')
        @yield('slider')

        @include('includes.partials.messages')
        @yield('content')

        @include('frontend.includes.v2.footer')
        @include('frontend.includes.v2.modals')
        @include('frontend.includes.v2.chat')
    </div><!-- #app -->
    ...
    <script type="text/javascript">
      ...
      jQuery(window).scroll(function() {
        ...
        $('#subscribe_form').on('submit', function(event) {
            //alert($('#subscriber_email').val());
            if($('#subscriber_email').val() == ''){
                $('#subscribe_form').parent().append('<div class="text-white">Please enter a valid email!</div>');
                return false;
            }
