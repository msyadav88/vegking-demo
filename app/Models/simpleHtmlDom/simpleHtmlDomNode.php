...
CONST HDOM_SMARTY_AS_TEXT = 1;
	...
	function innertext()
	{
		if (isset($this->_[self::HDOM_INFO_INNER])) {
			return $this->_[self::HDOM_INFO_INNER];
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case self::HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[self::HDOM_INFO_TEXT]);
			case self::HDOM_TYPE_COMMENT: return '';
			case self::HDOM_TYPE_UNKNOWN: return '';
		}
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			if ($val === true) {
				...
				switch ($this->_[self::HDOM_INFO_QUOTE][$i])
				{
				...
				$ret .= $key
				. $this->_[self::HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[self::HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
