...
//use App\Models\simpleHtmlDom\simpleHtmlDomNode;
...
//		die();
		...
		$defaultSpanText = self::DEFAULT_SPAN_TEXT)
	{
		...
		$this->root = new \App\Models\simpleHtmlDom\simpleHtmlDomNode($this);
		$this->root->tag = 'root';
	...
	protected function read_tag()
	{
		...
		do {
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[self::HDOM_INFO_END] = 0;
				$node->_[self::HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"':
				...
				break;
			case '\'':
				$quote_type = self::HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('\'');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			default:
				$quote_type = self::HDOM_QUOTE_NO;
		...
		if (!$is_duplicate) {
			$node->_[self::HDOM_INFO_QUOTE][] = $quote_type;
