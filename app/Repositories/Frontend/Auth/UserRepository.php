...
class UserRepository extends BaseRepository
{
     ...
     * @return mixed
     */
    public function findByConfirmationCode($code, $channel)
    {
        $user = $this->model
            ->where(($channel=='whatsapp'?'whatsapp_verification_code':'confirmation_code'), $code)
            ->first();
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            $user = parent::create([
                ...
                'whatsapp_verification_code' => md5(uniqid(mt_rand(), true)),
            ]);
	...
	public function updateimage($id, array $input, $image = false)
    {
		$id = $_POST['id'];
        $user = $this->getById($id);
		if($id){
			if ($image) {
				...
				$user->avatar_location = $input['avatar_location'];
			}
			return $user->save();
		}
    }
     ...
     * @return bool
     */
    public function confirm($code, $channel = 'email')
    {
        $user = $this->findByConfirmationCode($code, $channel);

        if (( $channel=='email' && $user->confirmed === true) || ($channel=='whatsapp' && $user->whatsapp_verified_at != NULL)) {
            throw new GeneralException(__('exceptions.frontend.auth.'.($channel=='whatsapp'?'whatsapp_verification':'confirmation').'.already_confirmed'));
        }
        $code_column = ($channel=='whatsapp'?'whatsapp_verification_code':'confirmation_code');
        if ($user->{$code_column} === $code) {
            if($channel=='whatsapp'){
               $user->whatsapp_verified_at = \Carbon\Carbon::now();
            }else{
               $user->confirmed = true;
            }
